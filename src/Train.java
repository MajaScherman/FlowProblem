...
public class Train {
 ...
 /*
Here we read in the file, first the number of nodes and then put all of the nodes in a list.
The nodes are just a name at a position in the list.

Then we read in the Arcs. A arc is a way from node u to v with a capacity c. The arc is not directed.
*/
	...
	//find edges from source
		for(int cur= 0; cur< nbrNodes; cur++){
			for(int i = 0; i< nbrArcs*2; i++){
				...
				if(resGraph.get(i).hasU(cur)){//if the arc (u,v) have cur = u add edge i
...
/*
	...
	Finding maximum flow through the min cut 
	source node s - ORIGIN
	sink node t - DESTINATIONS
	...
	*/
		...
		while(hasPath){
			...
			Path path = new Path(nodes.get(0), nodes.get(nbrNodes-1), nodes); //Path (s,t)
			hasPath = path.createSimplePath(resGraph); //P(G,f) men flow är en del i resGraph
